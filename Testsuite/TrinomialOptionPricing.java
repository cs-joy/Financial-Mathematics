...
public class TrinomialOptionPricing {
    ...
    public static final DoubleUnaryOperator CALL_PAYOFF = (price) -> Math.max(price - 100, 0);
    public static final DoubleUnaryOperator PUT_PAYOFF = (price) -> Math.max(100 - price, 0);
    ...
    public static void main(String[] args) {
        try {
            // Create a sample stock price tree (from previous implementation)
            double[][] stockTree = StockPriceTree.generateStockPrices(100.0, 3, 0.1);
            
            // Price a call option
            double callPrice = priceOption(stockTree, CALL_PAYOFF, 0.05, 0.3, 0.25, 0.1);
            System.out.printf("Call option price: %.4f%n", callPrice);
            
            // Price a put option
            double putPrice = priceOption(stockTree, PUT_PAYOFF, 0.05, 0.3, 0.25, 0.1);
